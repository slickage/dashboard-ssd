defmodule DashboardSSDWeb.LoginLive do
  @moduledoc """
  Login page with Google OAuth authentication.

    - Redirects authenticated users away from the login page.
  - Captures desired post-auth redirects and feeds them into the OAuth request.
  - Triggers the OAuth popup via LiveView events for a smooth UX.
  """
  use DashboardSSDWeb, :live_view

  @impl true
  def mount(params, session, socket) do
    # If user is already logged in, redirect to home
    if socket.assigns[:current_user] do
      {:ok, push_navigate(socket, to: "/")}
    else
      # Get redirect_to from URL params or session
      redirect_to = params["redirect_to"] || Map.get(session, "redirect_to")

      {:ok,
       socket
       |> assign(:page_title, "Sign In")
       |> assign(:redirect_to, redirect_to)}
    end
  end

  @impl true
  def handle_event("start_oauth", _params, socket) do
    redirect_to = socket.assigns[:redirect_to]
    oauth_url = ~p"/auth/google?#{%{redirect_to: redirect_to, popup: "true"}}"

    {:noreply,
     socket
     |> push_event("open_oauth_popup", %{url: oauth_url})}
  end

  @impl true
  def render(assigns) do
    ~H"""
    <div class="min-h-screen flex items-center justify-center bg-theme-background px-4">
      <div class="max-w-md w-full space-y-8">
        <!-- Logo/Brand -->
        <div class="text-center">
          <div class="flex justify-center">
            <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-theme-primary text-white text-2xl font-bold">
              DS
            </div>
          </div>
          <h2 class="mt-6 text-3xl font-bold text-theme-text">
            Welcome to DashboardSSD
          </h2>
          <p class="mt-2 text-sm text-theme-text-muted">
            Sign in to access your dashboard
          </p>
        </div>
        
    <!-- Login Form -->
        <div class="bg-theme-surface rounded-2xl shadow-theme-card p-8 border border-theme-border">
          <div class="space-y-6">
            <!-- Google OAuth Button -->
            <div>
              <button
                type="button"
                phx-click="start_oauth"
                class="w-full flex items-center justify-center gap-3 px-4 py-3 border border-theme-border rounded-xl text-theme-text bg-theme-surface hover:bg-white/10 transition-colors duration-200 group"
              >
                <svg class="h-5 w-5" viewBox="0 0 24 24">
                  <path
                    fill="#4285F4"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  />
                  <path
                    fill="#34A853"
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  />
                  <path
                    fill="#FBBC05"
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  />
                  <path
                    fill="#EA4335"
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  />
                </svg>
                <span class="text-sm font-medium">Continue with Google</span>
              </button>
            </div>
            
    <!-- Additional Info -->
            <div class="text-center">
              <p class="text-xs text-theme-text-muted">
                By signing in, you agree to our terms of service and privacy policy.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
    """
  end
end
