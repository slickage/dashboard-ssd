...
- [ ] T046 Run format and lint for repo (mix format; mix credo --strict)
 - [X] T057 [P] Make Meetings date range selectable (prev/next range + custom picker) (lib/dashboard_ssd_web/live/meetings_live/index.ex)
 - [X] T058 [P] Add simplified month calendar view with current-day highlight (lib/dashboard_ssd_web/components/calendar.ex)
 - [X] T059 [P] Show meeting times in the user's current timezone (lib/dashboard_ssd_web/date_helpers.ex)
 - [X] T060 [P] Highlight meetings happening today and label date as "Today" (lib/dashboard_ssd_web/live/meetings_live/index.ex)
