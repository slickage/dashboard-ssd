defmodule DashboardSSD.Integrations.GoogleCalendarCacheTest do
  ...
  test "caches events for same window and user" do
    ...
    assert {:ok, first} = Integrations.calendar_list_upcoming_for_user(user.id, now, later, ttl: 60_000)
    assert [%{id: id1}] = first

    # Second call should hit cache; mock would increment to evt-2 if invoked
    assert {:ok, second} = Integrations.calendar_list_upcoming_for_user(user.id, now, later, ttl: 60_000)
